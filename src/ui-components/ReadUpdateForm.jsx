...
export default function ReadUpdateForm(props) {
  const {
    id: idProp,
    read: readModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = readRecord
      ? { ...initialValues, ...readRecord }
      : initialValues;
    setTodoId(cleanValues.todoId);
    setReaderId(cleanValues.readerId);
    setErrors({});
  };
  const [readRecord, setReadRecord] = React.useState(readModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Read, idProp)
        : readModelProp;
      setReadRecord(record);
    };
    queryData();
  }, [idProp, readModelProp]);
  React.useEffect(resetStateValues, [readRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Read.copyOf(readRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ReadUpdateForm")}
      {...rest}
    >
      <TextField
        label="Todo id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || readModelProp)}
